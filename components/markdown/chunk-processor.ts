import { CharMatcher, Chunk, LineType } from "./types";

const matchers: CharMatcher[] = [
...
function matchLineType(line: string, index: number, matchers: CharMatcher[]) {
...
export class ChunkProcessor {
  chunks: Chunk[] = [];
  cacheLine: Chunk = { content: "", type: "unknown" };
  endPos: number = 0;
  ...
  private getLastChunk(): Chunk {
    return this.chunks[this.chunks.length - 1];
  }

  private mergeLineToLastChunk(line: Chunk, type?: LineType): Chunk {
  ...
  private addLineAsNewChunk(line: Chunk, type?: LineType): Chunk {
    if (type) {
      line.type = type;
    }
  ...
  private generateEmptyLine(): Chunk {
    return { content: "", type: "unknown" };
  }

  private handleNewLine(line: Chunk): Chunk {
  ...
  private handleUnknownLine(line: Chunk): Chunk {
  ...
  private handleKnownLine(line: Chunk): Chunk {
    this.updateCount ||= 1;
    return line;
  }

  private handleCodeBlockContent(line: Chunk): Chunk {
  ...
  private handleCodeBlockStart(line: Chunk): Chunk {
    this.setCodeBlockState(line);
    this.addCodeLoadingMark(line);
    return this.addLineAsNewChunk(line);
  }

  private handleNormalContent(line: Chunk): Chunk {
