...
export const MemoizedMarkdownComponent = memo(({ content }: { content: string }) => {
  return <MarkdownComponent content={content} />;
});

MemoizedMarkdownComponent.displayName = "MemoizedMarkdownComponent";
  ...
  return (
    <ReactMarkdown
      ...
      components={{
        ...
        h5: ({ children }) => (
          <h5 className="text-base font-semibold text-gray-900 mb-2 mt-3 first:mt-0">{children}</h5>
        ),
        ...
        pre: ({ children }) => {
          ...
          // Mermaid图表渲染
          if (language === "mermaid") {
            return <MermaidRenderer chart={codeString} loading={isLoading} />;
          }
        ...
        li: ({ children }) => {
          ...
          const hasCheckbox =
            ...
            ((children_array[0] as React.ReactElement<{ type: { name: string } }>).type as { name: string }).name ==
              "input";
